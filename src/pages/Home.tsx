...
const Home: React.FC = () => {
  ...
  const services = [
    {
      ...
      description:
        "Stunning, conversion-focused designs that captivate your audience and drive business results.",
      icon: Palette,
    ...
    },
    {
      ...
      description:
        "Full-featured e-commerce platforms that convert visitors into loyal customers.",
      icon: ShoppingCart,
    ...
    },
    {
      ...
      description:
        "Scalable web applications that streamline your business processes and enhance productivity.",
      icon: Laptop,
    ...
    },
    {
      ...
      description:
        "Easy-to-use content management systems that empower your team to update content effortlessly.",
      icon: Database,
    ...
    },
    {
      ...
      description:
        "Scalable APIs and backend solutions that power your applications and integrations.",
      icon: Server,
    ...
    },
    {
      ...
      description:
        "Optimize your website for lightning-fast performance and maximum conversions.",
      icon: Zap,
  ...
  const featuredProjects = [
    {
      ...
      industry: "SaaS",
      description:
        "AI-powered project management platform with real-time collaboration",
      image:
    ...
    },
    {
      ...
      industry: "E-Commerce",
      description:
        "Sustainable products marketplace with carbon footprint tracking",
      image:
  ...
  const testimonials = [
    {
      ...
      quote:
        "Pixeloria transformed our vision into a stunning reality. The attention to detail and technical expertise exceeded our expectations.",
      fullQuote:
        "Working with Pixeloria was a game-changer for our business. They didn't just build us a website; they created a digital experience that our users love. The team's attention to detail, technical expertise, and commitment to our success exceeded all expectations. Our conversion rates increased by 150% within the first month of launch.",
      rating: 5,
    ...
    },
    {
      ...
      quote:
        "The e-commerce platform they built for us is not just beautiful, but incredibly functional. Sales increased by 200% in the first quarter.",
      fullQuote:
    ...
    },
    {
      ...
      quote:
        "Their technical expertise in building our AI-powered dashboard was exceptional. The real-time analytics are exactly what we needed.",
      fullQuote:
        "Building a fintech product requires the highest standards of security and performance. Pixeloria delivered on both fronts while creating an intuitive user experience that our clients love. The real-time analytics dashboard they built has become the cornerstone of our platform.",
      rating: 5,
    ...
    },
    {
      ...
      quote:
        "HIPAA-compliant development with excellent UX. The telemedicine features work flawlessly across all devices.",
      fullQuote:
        "Healthcare technology demands perfection, and Pixeloria delivered exactly that. The patient portal they built is not only HIPAA-compliant but also incredibly user-friendly. The telemedicine integration has revolutionized how we connect with our patients.",
      rating: 5,
  ...
  const getCurrentMetrics = () => {
    if (homeSettings?.edge_numbers) {
      return [
        {
          number: homeSettings.edge_numbers.projects_delivered || 50,
          label: "Projects Delivered",
          suffix: "+",
        },
        {
          number: homeSettings.edge_numbers.client_satisfaction || 99,
          label: "Client Satisfaction",
          suffix: "%",
        },
        {
          number: homeSettings.edge_numbers.users_reached || 1000,
          label: "Users Reached",
          suffix: "",
        },
        {
          number: homeSettings.edge_numbers.support_hours || 24,
          label: "Support",
          suffix: "/7",
        },
      ];
  ...
  // CORRECTED: Count-up animation with cleanup to prevent memory leaks
  useEffect(() => {
    const currentMetrics = getCurrentMetrics();
    const timers: NodeJS.Timeout[] = []; // Array to store timer IDs for cleanup

    if (statsInView && (!countsStarted || homeSettings)) {
      setCountsStarted(true);
      setCounts(new Array(currentMetrics.length).fill(0)); // Initialize counts array

      currentMetrics.forEach((metric, index) => {
        let start = 0;
        const end =
          typeof metric.number === "string"
            ? parseFloat(metric.number) || 0
            : metric.number;
        const duration = 2000;
        ...
        timers.push(timer); // Store the timer ID for cleanup
      });
    }
  ...
  const nextTestimonial = () => {
    setCurrentTestimonial(
      (prev) => (prev + 1) % (filteredTestimonials.length || 1)
    );
  };
  ...
  const prevTestimonial = () => {
    setCurrentTestimonial(
      (prev) =>
        (prev - 1 + (filteredTestimonials.length || 1)) %
    ...
    const fetchData = async () => {
      try {
        ...
        const [portfolioResponse, servicesResponse, homeSettingsResponse] =
          await Promise.all([
            portfolioApi.getAll({ limit: 4 }).catch((err) => {
              console.warn("Portfolio API failed:", err);
            ...
            adminApi.getHomeSettings().catch((err) => {
              console.warn("Home settings API failed:", err);
              return { success: false, error: err.message };
            }),
          ]);

        // CORRECTED: Handle portfolio data with type narrowing
        if (portfolioResponse.success) {
          ...
          console.error("Portfolio API error:", portfolioResponse.error);
        }

        // CORRECTED: Handle services data with type narrowing
        if (servicesResponse.success) {
          setApiServices(servicesResponse.data?.services || []);
        } else {
          console.error("Services API error:", servicesResponse.error);
        }

        // CORRECTED: Handle home settings data with type narrowing
        if (homeSettingsResponse.success) {
          const homeSettingsData =
            homeSettingsResponse.data?.homeSettings || homeSettingsResponse.data;

          if (homeSettingsData && typeof homeSettingsData === "object") {
            setHomeSettings(homeSettingsData);

            setTimeout(() => {
              setCountsStarted(false);
              setCounts([0, 0, 0, 0]);
            }, 0);

            if (
              homeSettingsData.featured_case_studies &&
              Array.isArray(homeSettingsData.featured_case_studies)
            ) {
              const featuredProjectIds = homeSettingsData.featured_case_studies
                .map((cs: any) =>
                  typeof cs === "object" ? cs.portfolio_id : cs
                )
                .filter(Boolean);

              if (featuredProjectIds.length > 0) {
                // Check if portfolioResponse was successful before accessing its data
                if (portfolioResponse.success) {
                  const matchedProjects =
                    portfolioResponse.data.projects.filter((project: any) =>
                      featuredProjectIds.includes(project.id)
                    );
                  setFeaturedCaseStudies(matchedProjects);
                } else {
  ...
  const filteredProjects =
    projectFilter === "All"
      ...
      : (featuredCaseStudies.length > 0
          ? featuredCaseStudies
          : featuredProjects
        ).filter((project) => project.category === projectFilter);
  ...
  const filteredTestimonials =
    testimonialFilter === "All"
      ...
      : (featuredTestimonials.length > 0
          ? featuredTestimonials
          : testimonials
        ).filter((testimonial) => testimonial.industry === testimonialFilter);

  // Auto-advance testimonials
  useEffect(() => {
    const interval = setInterval(nextTestimonial, 5000);
    return () => clearInterval(interval);
  }, [filteredTestimonials.length]); // Depend on the length to restart if filters change

  // Effect to handle homeSettings changes specifically for the count-up animation
  useEffect(() => {
    if (homeSettings?.edge_numbers) {
      setCountsStarted(false);
      setCounts([0, 0, 0, 0]);
  ...
  // CORRECTED: Safely get the active testimonial to prevent runtime crashes
  const activeTestimonial = filteredTestimonials[currentTestimonial];
                ...
                >
                  ...
                  <div className="relative p-8 h-full flex flex-col">
                    ...
                    <div className="flex flex-wrap gap-2 mb-4">
                      {service.techStack
                        .slice(0, selectedService === service.id ? 4 : 2)
                        .map((tech, techIndex) => (
                          <motion.span
                            key={tech}
              ...
              >
                ...
                >
                  ...
                  <div className="relative z-10">
                    ...
                    <p
                      className={`text-lg font-semibold mb-2 bg-gradient-to-r ${item.color} bg-clip-text text-transparent`}
                    >
                      {item.tagline}
                    </p>
        ...
        <div className="container-custom">
          ...
          <div className="relative">
            <AnimatePresence mode="wait">
              {/* CORRECTED: Check if activeTestimonial exists before rendering */}
              {activeTestimonial ? (
                <motion.div
                ...
                <div className="text-center py-10">
                  <p className="text-gray-600">
                    No testimonials available for this filter.
                  </p>
                </div>
              )}
            </AnimatePresence>
        ...
        <div className="container-custom text-center relative z-10">
          ...
          >
            ...
            <p className="text-xl text-blue-100 mb-8 max-w-2xl mx-auto">
              Let's create something extraordinary together. Join the growing
              list of successful businesses we've helped transform.
            </p>
